...
pair<int, int> near[4] = { make_pair(-1, 0), make_pair(0, -1), make_pair(0, 1), make_pair(1, 0) };
char dir[4] = { 'U', 'L', 'R', 'D' };
...
string solve(const vector<vector<char>>& board, pair<int, int> R, pair<int, int> B) {		//DFS
	queue<pair<int, int>> rq;	//R과 인접한 칸들
	queue<pair<int, int>> bq;	//R이 rq에서 i번째 위치일 때, B의 위치
	queue<string> moved_dir;	//움직여온 방향을 저장
	...
	while (!rq.empty()) {
		...
		string pre_dirs = moved_dir.front();//이번 방문을 위해 이전까지 움직였던 방향

		rq.pop();
		bq.pop();
		moved_dir.pop();

		if (pre_dirs.size() > 9 || visited[r_temp.first][r_temp.second][b_temp.first][b_temp.second])	//움직인 숫자가 10보다 크거나
			continue;																					//이미 해당 부분(r과 b 모두 같은위치)을 방문했다면 continue
		...
		for (int i = 0; i < 4; i++) {
			...
			if (!visited[mr.first][mr.second][mb.first][mb.second]	//움직인 R과 B가 방문한 적이 없고,
				...
				if (board[mb.first][mb.second] == 'O')				//움직인 B가 구멍에 빠지면 continue
					...
					return pre_dirs + dir[i];
				}
				else {												//어디 빠진게 없으면 다음 경로에 넣어줌
					rq.push(mr);
					bq.push(mb);
					moved_dir.push(pre_dirs + dir[i]);
				}
			}
		}
	}
